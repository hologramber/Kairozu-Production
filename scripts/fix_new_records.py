...
for prof in profiles:
    ...
    if len(vocabulary) == 0:
        ...
        for vocab in vocabulary:
            VocabRecord.objects.get_or_create(user=prof.user, vocab=vocab)
    ...
    if len(expressions) == 0:
        ...
        for express in expressions:
            ExpressionRecord.objects.get_or_create(user=prof.user, express=express)
    ...
    if len(sentences) == 0:
        ...
        for sent in sentences:
            SentenceRecord.objects.get_or_create(user=prof.user, sentence=sent)
