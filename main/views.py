...
class ProgressView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        vocabs = VocabRecord.objects.filter(user_id=self.request.user.id, rating__gt=0, score__lte=2).order_by('-score')[:100]
        expressions = ExpressionRecord.objects.filter(user_id=self.request.user.id, rating__gt=0, score__lte=2).order_by('-score')[:50]
        sentences = SentenceRecord.objects.filter(user_id=self.request.user.id, rating__gt=0, score__lte=2).order_by('-score')[:150]
